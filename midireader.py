...
if(input_id == -1):
    ...
    while True: #This loop only terminates when knob 4 of AKAI MPK mini is rotated, configure for differet MIDI
        ...
        if(new_key==1):                    #refreshes the location when a new key is pressd
            penup()                        #ensures that when the pen is moving it doesn't draw
            x=(key*velocity^2) % 300 - 300 #calculates the new x value
            y=(key*velocity) % 300 - 300   #calculates the new y value
